...
router.get('/print-cart', function(req,res,next){
    var sql1 = "select user.ID as userID, user.name as user_name, menu.name as menu_name, menu.id as menuID, menu.price as menu_price from user_menu inner join menu on menu_ID = menu.ID inner join user on user_ID = user.id; select user.name as user_name, sum(menu.price) as total_price from user_menu inner join menu on menu_ID = menu.ID inner join user on user_ID = user.id group by user.id";
    ...
    multiconnection.query(sql1, [2, 1], function(err,row){
			if(err) console.error("err: "+err);
			console.log("장바구니 조회");
...
router.get('/buy/:userID', function(req, res, next){
    var userID = req.params.userID;
    console.log(userID);
    var sql1 = "select user.name as user_name, menu.name as menu_name, menu.price as menu_price from user_menu inner join menu on menu_ID = menu.ID inner join user on user_ID = user.id";
    var sql2 = ""
    res.render('buy', {title: "구매"});
});
